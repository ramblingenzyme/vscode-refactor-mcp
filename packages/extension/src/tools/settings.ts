...
const getSettingSchema = z.object({
  ...
  scope: z.enum(['workspace', 'user']).optional().describe('The configuration scope (defaults to workspace)'),
});
...
export const getSettingTool: Tool = {
  ...
  async implementation(args: any): Promise<CallToolResult> {
    try {
      ...
      let value: any;
      if (isWorkspaceScope) {
        value = inspect.workspaceValue ?? inspect.globalValue ?? inspect.defaultValue;
      } else {
