    ...
    try {
        ...
        if (registrationResult.skipped) {
            ...
            if (registrationResult.registered.length > 0) {
                logger.info(`Registered ${registrationResult.registered.length} tools: ${registrationResult.registered.join(', ')}`);
            }
            
            if (registrationResult.failed.length > 0) {
                logger.error(`Failed to register ${registrationResult.failed.length} tools:`);
        ...
        if (config.enableHttpServer) {
            try {
                ...
                if (!isValidPort(config.httpPort)) {
                    throw new Error(`Invalid port number: ${config.httpPort}. Port must be 0 or between 1 and 65535.`);
                }
